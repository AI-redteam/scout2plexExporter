...
detailed descriptions, accurate asset identification, and robust filtering.
"""
...
class ScoutSuiteToPlexTrac:
    ...
    def _get_asset_details(self, scout_data, item_path):
        ...
        # --- Definitive Region Detection Logic ---
        # 1. Try to parse from the path first (most reliable for regional services like EC2/RDS).
        region_match = re.search(r'\.regions\.([\w-]+)\.', item_path)
        region = region_match.group(1) if region_match else None
        # 2. If not in path (like for S3), look for a 'region' key in the resource's data.
        if not region:
            region = resource_obj.get('region', resource_obj.get('Region'))
        # 3. If still no region is found, default to 'global'.
        if not region:
            region = 'global'
